...
s5 = "kapoor-rakshit@github.co.in"

reg_pattern = r"(\w+.\w+.\w+)@(\w+)\.(\w+.\w+)"

m = re.match(reg_pattern , s5)       # re.match() expression only matches at the beginning of the string. 
...
reg_patterndict = r"(?P<username>\w+.\w+.\w+)@(?P<website>\w+)\.(?P<domain>\w+.\w+)"
